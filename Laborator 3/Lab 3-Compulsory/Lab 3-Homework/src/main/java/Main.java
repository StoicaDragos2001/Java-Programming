...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("Sorting for the nodes which are identifiable: ");
        List<Node> identifiableNodes;
        identifiableNodes = network.showIdentifiableNodes();
        for (int index1 = 0; index1 < identifiableNodes.size(); index1++) {
            for (int index2 = 0; index2 < identifiableNodes.size(); index2++) {
                identifiableNodes.get(index1).displayWithCosts(identifiableNodes.get(index1), identifiableNodes.get(index2));
            }
        }
        TravelAlgorithm algorithm = new TravelAlgorithm();
        algorithm.createMatrix(identifiableNodes);
    }
}
